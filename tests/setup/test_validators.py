...
class TestSetupValidators:
    ...
    def test_domain_validation_format(self):
        ...
        for domain in invalid_domains:
            assert not re.match(domain_pattern, domain), f"Invalid domain passed: {domain}"
    ...
    def test_docker_version_parsing(self):
        ...
        for current_version, min_version, expected in test_versions:
            ...
            assert result == expected, f"Version {current_version} >= {min_version} should be {expected}"
...
"""
    ...
    def test_ssl_configuration_validation(self):
        ...
        ssl_configs = [
            {"enable_ssl": "true", "domain": "example.com", "email": "test@example.com", "valid": True},
            {"enable_ssl": "true", "domain": "", "email": "test@example.com", "valid": False},
            {"enable_ssl": "true", "domain": "example.com", "email": "", "valid": False},
        ...
        for config in ssl_configs:
            ...
            assert is_valid == config["valid"], f"SSL config validation failed for {config}"