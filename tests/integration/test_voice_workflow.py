...
class TestVoiceProcessingWorkflow:
    ...
    async def test_complete_voice_note_processing(
        ...
        upload_response = await async_client.post(
            "/api/v1/audio/upload", files=files, data=data
        )
    ...
    async def test_voice_workflow_with_session_management(
        ...
        files = {"file": ("test_audio.webm", test_audio_content, "audio/webm")}
        data = {"description": "Session-based voice note", "session_id": session_id}
        ...
        session_check_response = await async_client.get(
        ...
        edit_response = await async_client.put(
            f"/api/v1/sessions/{session_id}/update", json=edit_data
        )
    ...
    async def test_voice_workflow_error_recovery(
        ...
        with patch(
            "app.services.whisper_model.WhisperModel.transcribe"
        ) as mock_transcribe:
            # First call fails
            ...
            assert (
                "Recovered transcription after error"
                in retry_data["transcription"]["text"]
            )
    ...
    async def test_concurrent_voice_processing(
        ...
        async def process_voice_note(note_id: int):
            ...
            files = {
                "file": (f"test_audio_{note_id}.webm", test_audio_content, "audio/webm")
            }
    ...
    async def test_voice_workflow_with_editing(
        ...
        edit_summary_response = await async_client.put(
            f"/api/v1/sessions/{session_id}/update",
            json={"summary": custom_summary, "keywords": custom_keywords},
        )
