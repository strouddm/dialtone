...
class TestOllamaService:
    ...
    async def test_ensure_model_loaded_already_loaded(
        self, ollama_service, mock_client
    ):
    ...
    async def test_generate_summary_success(self, ollama_service, mock_client):
        ...
        with patch.object(ollama_service, "_get_client", return_value=mock_client):
            result = await ollama_service.generate_summary(
                "Test text content about a meeting with decisions"
            )
    ...
    async def test_generate_summary_empty_response(self, ollama_service, mock_client):
        ...
        with patch.object(ollama_service, "_get_client", return_value=mock_client):
            result = await ollama_service.generate_summary(
                "This is a test sentence about a meeting. We discussed important topics. Action items were assigned."
            )
    ...
    @pytest.mark.asyncio
    async def test_generate_summary_api_error_uses_fallback(
        self, ollama_service, mock_client
    ):
        ...
        with patch.object(ollama_service, "_get_client", return_value=mock_client):
            result = await ollama_service.generate_summary(
                "This is a test about a project meeting. Important decisions were made. We have action items."
            )
    ...
    async def test_generate_summary_with_retries(self, ollama_service, mock_client):
        ...
        success_response.json.return_value = {
            "response": "- Successful summary after retry\n- Key points identified"
        }
    ...
    @pytest.mark.asyncio
    async def test_generate_summary_max_words_parameter(
        self, ollama_service, mock_client
    ):
