...
class TestPerformanceBenchmarks:
    ...
    async def test_5_minute_audio_processing_time(
        ...
        large_audio_content = b'\x1a\x45\xdf\xa3' + b'\x00' * (five_minute_audio_size - 4)
        ...
        mock_whisper_service.transcribe.return_value = {
            ...
            "segments": [
                {"start": i * 30.0, "end": (i + 1) * 30.0, "text": f"Segment {i + 1} of the five-minute audio transcription."}
        ...
        mock_ollama_service.summarize.return_value = {
            ...
            "keywords": ["performance", "benchmark", "processing", "audio", "transcription"]
        }
        ...
        assert benchmark.stats.mean < 35.0, f"Processing took {benchmark.stats.mean:.2f}s, exceeds 35s requirement"
    ...
    async def test_concurrent_processing_performance(
        ...
        async def process_concurrent_audio():
            ...
            async def process_single_audio(audio_id: int):
                files = {"file": (f"concurrent_{audio_id}.webm", test_audio_content, "audio/webm")}
        ...
        assert benchmark.stats.mean < 60.0, f"Concurrent processing took {benchmark.stats.mean:.2f}s, too slow"
    ...
    async def test_memory_usage_under_load(
        ...
        mock_whisper_service.transcribe.return_value = {
            ...
            "segments": [
                {"start": i * 10.0, "end": (i + 1) * 10.0, "text": f"Memory test segment {i + 1} with detailed content."}
        ...
        def monitor_memory_usage():
            ...
            async def memory_stress_test():
                ...
                for i in range(3):
                    files = {"file": (f"memory_test_{i}.webm", large_audio_content, "audio/webm")}
        ...
        assert peak_memory < 16.0, f"Peak memory usage {peak_memory:.2f}GB exceeds 16GB requirement"
    ...
    async def test_api_response_times(
        ...
        for endpoint, response_time in response_times.items():
            assert response_time < 0.5, f"{endpoint} response time {response_time:.3f}s exceeds 500ms requirement"
    ...
    async def test_processing_throughput(
        ...
        async def throughput_test():
            ...
            for i in range(5):
                files = {"file": (f"throughput_test_{i}.webm", test_audio_content, "audio/webm")}
        ...
        def cleanup_test():
            ...
            async def process_and_cleanup():
                ...
                for i in range(10):
                    ...
                    files = {"file": (f"cleanup_test_{i}.webm", test_audio_content, "audio/webm")}
                    upload_response = await async_client.post(
                        "/api/v1/audio/upload",
                        files=files,
                        data={"session_id": session_data["session_id"], "description": f"Cleanup test {i}"}
                    )
                    upload_data = upload_response.json()
