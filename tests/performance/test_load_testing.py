...
class TestLoadTesting:
    ...
    async def test_high_volume_upload_load(
        ...
        # Analyze results
        successful_uploads = [r for r in results if isinstance(r, dict) and r.get("success", False)]
        failed_uploads = [r for r in results if not (isinstance(r, dict) and r.get("success", False))]
        
        # Assertions
        assert len(successful_uploads) >= 40, f"Only {len(successful_uploads)}/50 uploads succeeded"
        ...
        response_times = [r["response_time"] for r in successful_uploads if r.get("response_time")]
        if response_times:
    ...
    async def test_sustained_processing_load(
        ...
        async def process_audio_workflow(workflow_id: int):
            ...
            try:
                # Upload
                files = {"file": (f"sustained_{workflow_id}.webm", test_audio_content, "audio/webm")}
                ...
                if upload_response.status_code != 201:
                    return {"workflow_id": workflow_id, "success": False, "stage": "upload"}
                ...
                if transcription_response.status_code != 200:
                    return {"workflow_id": workflow_id, "success": False, "stage": "transcription"}
                
                # Summarize
                summary_response = await async_client.post(f"/api/v1/audio/{upload_id}/summarize")
                if summary_response.status_code != 200:
                    return {"workflow_id": workflow_id, "success": False, "stage": "summarization"}
                
                return {"workflow_id": workflow_id, "success": True, "upload_id": upload_id}
        ...
        while time.time() - start_time < duration:
            ...
            completed_workflows.extend([r for r in batch_results if isinstance(r, dict)])
            
            # Small delay between batches
            await asyncio.sleep(2)
        
        # Analyze sustained load results
        successful_workflows = [w for w in completed_workflows if w.get("success", False)]
        failed_workflows = [w for w in completed_workflows if not w.get("success", False)]
        
        total_workflows = len(completed_workflows)
        success_rate = len(successful_workflows) / total_workflows if total_workflows > 0 else 0
        ...
        assert len(failed_workflows) <= total_workflows * 0.1, "Too many failures during sustained load"
    ...
    async def test_session_management_load(
        ...
        async def session_workflow(session_id: int):
            ...
            try:
                ...
                if session_response.status_code != 201:
                    return {"session_id": session_id, "success": False, "stage": "create"}
                ...
                for update_num in range(3):
                    ...
                    update_response = await async_client.put(
                        f"/api/v1/sessions/{actual_session_id}/update",
                        json=update_data
                    )
                    
                    if update_response.status_code != 200:
                        return {"session_id": session_id, "success": False, "stage": f"update_{update_num}"}
                
                # Get session state
                get_response = await async_client.get(f"/api/v1/sessions/{actual_session_id}")
                if get_response.status_code != 200:
                    return {"session_id": session_id, "success": False, "stage": "get"}
                
                # Complete session
                complete_response = await async_client.post(f"/api/v1/sessions/{actual_session_id}/complete")
                if complete_response.status_code not in [200, 201]:
                    return {"session_id": session_id, "success": False, "stage": "complete"}
                
                return {"session_id": session_id, "success": True, "actual_session_id": actual_session_id}
        ...
        successful_sessions = [s for s in session_results if isinstance(s, dict) and s.get("success", False)]
        failed_sessions = [s for s in session_results if not (isinstance(s, dict) and s.get("success", False))]
        ...
        assert len(failed_sessions) <= 3, f"Too many session failures: {len(failed_sessions)}"
    ...
    async def test_mixed_workload_stress(
        ...
        async def upload_worker(worker_id: int):
            ...
            for i in range(5):
                ...
                audio_content = b'\x1a\x45\xdf\xa3' + b'\x00' * (size - 4)
                files = {"file": (f"mixed_{worker_id}_{i}.webm", audio_content, "audio/webm")}
                ...
                try:
                    ...
                    if session_response.status_code == 201:
                        ...
                        await async_client.put(
                            ...
                            json={"transcription": f"Mixed workload session {worker_id}-{i}"}
                        )
        ...
        total_health_checks = sum(health_checks) if all(isinstance(h, int) for h in health_checks) else 0
        total_uploads = sum(uploads) if all(isinstance(u, int) for u in uploads) else 0
        total_sessions = sum(sessions) if all(isinstance(s, int) for s in sessions) else 0
        
        # Assertions
        assert total_health_checks >= 35, f"Health checks: {total_health_checks}/40 passed"
    ...
    async def test_error_recovery_under_load(
        ...
        async def flaky_workflow(workflow_id: int):
            ...
            for attempt in range(max_retries):
                try:
                    # Upload
                    files = {"file": (f"flaky_{workflow_id}_{attempt}.webm", test_audio_content, "audio/webm")}
                    upload_response = await async_client.post(
                        "/api/v1/audio/upload",
                        files=files,
                        data={"description": f"Flaky workflow {workflow_id} attempt {attempt}"}
                    )
                    ...
                    if upload_response.status_code == 201:
                        ...
                        with patch("app.services.whisper_model.WhisperModel") as mock_whisper:
                            if random.random() < 0.3:  # 30% chance of failure
                                mock_whisper.return_value.transcribe.side_effect = Exception("Random failure")
                            else:
                            ...
                            if transcription_response.status_code == 200:
                                return {"workflow_id": workflow_id, "success": True, "attempts": attempt + 1}
                    ...
                    if attempt == max_retries - 1:
                        return {"workflow_id": workflow_id, "success": False, "error": str(e), "attempts": attempt + 1}
            
            return {"workflow_id": workflow_id, "success": False, "attempts": max_retries}
        ...
        successful_workflows = [w for w in flaky_results if isinstance(w, dict) and w.get("success", False)]
        failed_workflows = [w for w in flaky_results if isinstance(w, dict) and not w.get("success", False)]
        
        success_rate = len(successful_workflows) / len(flaky_results)
        avg_attempts = sum(w.get("attempts", 1) for w in flaky_results if isinstance(w, dict)) / len(flaky_results)
        
        # Assertions for error recovery
        assert success_rate >= 0.7, f"Error recovery success rate {success_rate:.2%} too low"
        assert avg_attempts <= 2.5, f"Average attempts {avg_attempts:.1f} too high"
        assert total_time < 30.0, f"Error recovery test took {total_time:.2f}s, too slow"